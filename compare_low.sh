...
OPTIONS:
    ...
    -s, --step NUM          Run only a specific step from 1 to 9 (e.g. 1 for 25%)
...
DESCRIPTION:
    ...
    Added two levels with 25% and 50% of load to show the similar
    behaviour of the two approaches during situations with no stress.
...
for i in "${!LEVELS[@]}"; do
    ...
    hey -z ${DURATION}s -q $qps http://localhost:8081 > ./metrics/low/rr_low_${i}.txt 2>&1

    echo "Waiting 5 seconds before switching..."
    sleep 5

    echo "2/2: Testing Prequal..."
    hey -z ${DURATION}s -q $qps http://localhost:8080 > ./metrics/low/pq_low_${i}.txt 2>&1
    ...
    grep -E "Requests/sec:|p50|p99|p99.9" ./metrics/low/rr_low_${i}.txt | head -5

    echo ""
    echo "--- Prequal Results ---"
    grep -E "Requests/sec:|p50|p99|p99.9" ./metrics/low/pq_low_${i}.txt | head -5
...
echo ""
echo "Detailed results saved in ./metrics/low/rr_low_*.txt and ./metrics/low/pq_low_*.txt"
